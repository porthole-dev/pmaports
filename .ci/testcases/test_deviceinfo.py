...
def test_deviceinfo():
    ...
    for folder in pkgrepo_iglob("device/*/device-*"):
        ...
        with open(folder / "deviceinfo") as f:
            lines = f.read().split("\n")
        ...
        try:
            for line in lines:
                ...
                if not pattern.match(line) or line.endswith('\\"'):
                    raise RuntimeError(
                        "Line looks invalid, maybe missing"
            ...
            if (
                not name.startswith(manufacturer)
                and not name.startswith("Google")
                and not name.startswith("Jolla")
            ):
                raise RuntimeError(
                    "Please add the manufacturer in front of"
                    " the deviceinfo_name, e.g.: '" + manufacturer + " " + name + "'"
                )
